...
src/components/       通用组件 (Menu / TextInput / Message / ProgressBar / BackToMenu)
...
- 长任务 running 界面统一用 **ProgressBar** (`src/components/ProgressBar.tsx`): props `{ percent: number | null; width?: number; extra?: string }`, `percent=null` 时显示 `--.-%` 占位 (总时长未知), `extra` 追加速度等信息 (如 `| 1.5x`). 三个命令共用.
- **ffmpeg 任务** (to-h265 / cut-video): 命令加 `-nostats -progress pipe:1` 参数, 用 `runFfmpeg` 的 `onProgress` 回调拿到 `outTimeMs`; 分母用 `probeDuration()` (ffprobe `format=duration`), 百分比 = `outTimeMs / durationMs`. cut-video 的裁剪时长按 `总时长 - 起始时间` 估算 (fast seek 有偏差, 仅用于进度). 起始时间解析用 `parseTimeToMs` (ffmpeg.ts 导出).
- **dd 任务** (copy-to-hdd): 用 `setInterval` 轮询 `stat` 目标文件大小算字节进度 + 速度, 不支持进度来源时走 `lastError` 提示.
...
- **ffmpeg 相关命令优先真机复测** (容器已装真实 ffmpeg): fake shim 与真实输出有细节差异 (曾因 fake 输出"真毫秒"而漏掉 out_time_ms 的微秒坑, 见常见坑 8). 容器没有的话可写 fake shim 冒烟 (见 ink-tui-testing 记忆).
...
8. ffmpeg `-progress` 输出的 `out_time_ms` 在 6.x 实际是微秒 (与 `out_time_us` 同值), 用它算进度会立刻 100%. 进度解析一律用 `out_time` 字符串 (`HH:MM:SS.ffffff`), 见 ffmpeg.ts 的 `onProgress`. `speed` 首块可能是 `N/A` 需过滤 (渲染处用正则 `/^\d+(\.\d+)?x$/` 判断).
